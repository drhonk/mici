 ...
 * Begin Document
 */
class RestController extends MI_Controller
{
    ...
    function __construct()
    {
        ...
        // Load required plugins, config & modules
        $this->load->library('pear_doctrine');
        $this->load->library('security');
        $this->load->config('rest');
        $this->load->model('codeigniter/auth/apikey_model');
        ...
        // Validate API Key
        $apikey = (isset($this->_args['apikey']))
                ? $this->_args['apikey']
                : NULL;

        $this->_validate_api_key($apikey);
    }
    ...
    {
        return array_key_exists($key, $this->_get_args)
                ? $this->_xss_clean($this->_get_args[$key], $xss_clean)
    ...
    {
        return array_key_exists($key, $this->_put_args)
                ? $this->_xss_clean($this->_put_args[$key], $xss_clean)
                : FALSE;
    }

    public function delete($key, $xss_clean = TRUE)
    {
        return array_key_exists($key, $this->_delete_args)
                ? $this->_xss_clean($this->_delete_args[$key], $xss_clean)
                : FALSE;
    }
    ...
    private function _validate_api_key($key)
    {
        ...
        else
        {
            ...
            $keys = new Apikey_model();
            $check = $keys->validate_key($key);
            if ($check !== TRUE)
            {
                $error = (is_string($check))
                        ? $check
                        : 'Invalid API Key.';
    ...
    private function _format_csv($data = array())
    {
        ...
        $this->output->set_header("Content-Disposition: attachment; filename=response.{$this->_format}");
